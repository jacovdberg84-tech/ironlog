...
export default async function dashboardRoutes(app) {
  ...
  app.get("/", async (req, reply) => {
    ...
    const assetRows = getDayAssetHours.all(date);
    const downtimeRows = getDayAssetDowntime.all(date);
    ...
    assetRows.forEach((r) => {
      ...
      // Vehicle rows can be captured as km/day in daily input. Convert to equivalent hours
      // for utilization math while preserving normal hour-based assets.
      const run = mode === "km" ? (runRaw / kmPerHour) : runRaw;
      const loggedDown = Math.max(0, Number(downtimeByAsset.get(assetId) || 0));
      // Per-asset cap guarantees a full-day breakdown can zero availability
      // without inflating totals across assets.
      const cappedDown = Math.min(loggedDown, scheduled);
      // KPI: availability should include all active, non-standby used assets.
      // Utilization math supports km-mode by converting km/day to equivalent hours.
      const contributes_to_kpi = true;
      scheduled_hours += scheduled;
      run_hours += Math.min(run, scheduled);
      ...
      per_asset_kpi.push({
        ...
        run_hours: Number(Math.min(run, scheduled).toFixed(2)),
        downtime_hours: Number(cappedDown.toFixed(2)),
        available_hours: Number(Math.max(0, scheduled - cappedDown).toFixed(2)),
      });
    });

    // If we have downtime logs for assets that do NOT have a daily_hours row,
    // include them using the scheduled fallback. This prevents "Major Downtime"
    // showing hours while KPI downtime remains 0.
    ...
    const availability =
      scheduled_hours > 0 ? (available_hours / scheduled_hours) * 100 : null;

    const utilization =
      available_hours > 0 ? (run_hours / available_hours) * 100 : null;

    // "Used assets" should reflect the same pool used for KPI math.
    // We count any asset that has any scheduled/run/downtime contribution for the day.
    const used_assets = Array.from(
      new Set(
        per_asset_kpi
          .filter((r) => {
            const s = Number(r.scheduled_hours || 0);
            const run = Number(r.run_hours || 0);
            const down = Number(r.downtime_hours || 0);
            return s > 0 || run > 0 || down > 0;
          })
          .map((r) => Number(r.asset_id || 0))
      )
    ).length;
  ...
  // Body: { asset_code, log_date?, liters, hours_run?, source?, force_duplicate? }
  app.post("/fuel/log", async (req, reply) => {
    ...
    if (!forceDuplicate) {
      const recentDup = db.prepare(`
        ...
        ORDER BY id DESC
        LIMIT 1
      `).get(asset.id, log_date, liters, hours_run, hours_run, source);
    ...
    const ins = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(asset.id, log_date, liters, source, hours_run);
    ...
    writeAudit(db, req, {
      ...
      entity_type: "asset",
      entity_id: asset_code,
      payload: { log_date, liters, hours_run, source },
    });
  ...
  // Body: { asset_code, baseline_fuel_l_per_hour }
  app.post("/fuel/baseline", async (req, reply) => {
    if (!requireRoles(req, reply, ["admin", "supervisor"])) return;
    const body = req.body || {};
    const asset_code = String(body.asset_code || "").trim();
    const baseline = Number(body.baseline_fuel_l_per_hour ?? 0);

    if (!asset_code) return reply.code(400).send({ error: "asset_code is required" });
    if (!Number.isFinite(baseline) || baseline <= 0) {
      return reply.code(400).send({ error: "baseline_fuel_l_per_hour must be > 0" });
    }

    const asset = db.prepare(`SELECT id, asset_code, asset_name FROM assets WHERE asset_code = ?`).get(asset_code);
    if (!asset) return reply.code(404).send({ error: `asset_code not found: ${asset_code}` });
    ...
    writeAudit(db, req, {
      ...
      entity_type: "asset",
      entity_id: asset_code,
      payload: { baseline_fuel_l_per_hour: baseline },
    });
    ...
    return reply.send({
      ...
      baseline_fuel_l_per_hour: Number(baseline.toFixed(3)),
    });
  });
  ...
  app.get("/fuel", async (req, reply) => {
    ...
    const getHoursFuel = db.prepare(`
      SELECT
        COALESCE(SUM(
          CASE
            WHEN open_meter_value IS NOT NULL
              AND close_meter_value IS NOT NULL
              AND close_meter_value > open_meter_value
            THEN (close_meter_value - open_meter_value)
            ELSE CASE WHEN hours_run > 0 THEN hours_run ELSE 0 END
          END
        ), 0) AS hours_run
      FROM fuel_logs
      WHERE asset_id = ?
    ...
    const summary = rows.reduce(
      ...
      { assets: 0, fuel_liters: 0, hours_run: 0, excessive_count: 0 }
    );
  ...
  // Returns one row per fuel fill entry; L/hr uses hours between fills.
  app.get("/fuel/daily", async (req, reply) => {
    ...
    const rows = fuelRows.map((d) => {
      ...
      const isExcessive = lph != null && lph > threshold;
      if (hasRowMeters && rowClose > 0) prevMeter = rowClose;
      else if (meter > 0) prevMeter = meter;
      return {
        id: Number(d.id),
        log_date: d.log_date,
        fuel_liters: Number(fuel.toFixed(2)),
        hours_run: hoursBetween == null ? 0 : Number(hoursBetween.toFixed(2)),
        meter_hours: meter > 0 ? Number(meter.toFixed(2)) : null,
        actual_lph: lph == null ? null : Number(lph.toFixed(3)),
        oem_lph: Number(oem.toFixed(3)),
    ...
    const summary = rows.reduce(
      ...
      { days: 0, fuel_liters: 0, hours_run: 0, excessive_days: 0 }
    );
    summary.fuel_liters = Number(summary.fuel_liters.toFixed(2));
    summary.hours_run = Number(summary.hours_run.toFixed(2));
    summary.avg_lph = summary.hours_run > 0
