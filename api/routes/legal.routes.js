...
export default async function legalRoutes(app) {
  ...
  app.post("/upload", async (req, reply) => {
    ...
    // Parse all parts so upload works regardless of multipart field order.
    const fields = {};
    let filePart = null;
    let buf = null;
    for await (const part of req.parts()) {
      if (part.type === "file") {
        if (!filePart) {
          ...
          // Drain extra file streams if any.
          part.file.resume();
        }
      } else {
    ...
    if (!filePart || !buf) {
      return reply.code(400).send({ error: "Upload a file field named 'file'." });
    }

    const original = String(filePart.filename || "document.bin");
    const titleFromFile = original.replace(/\.[^.]+$/, "").trim();
    const department = String(fields.department || "").trim() || DEFAULT_DEPARTMENTS[0] || "General";
    const title = String(fields.title || "").trim() || titleFromFile || "Untitled";
    const doc_type = String(fields.doc_type || "").trim() || null;
    const version = String(fields.version || "").trim() || null;
    const owner = String(fields.owner || "").trim() || null;
    const effective_date = String(fields.effective_date || "").trim() || null;
    const expiry_date = String(fields.expiry_date || "").trim() || null;
    ...
    `).run(
      ...
      original,
      relPath,
      String(filePart.mimetype || "").trim() || null,
      Number(buf.length || 0),
      uploaded_by
    );
