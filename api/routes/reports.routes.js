...
function kpiDaily(date, scheduled) {
  ...
  const dtCol = getBreakdownDowntimeColumn();
  const dtRow = db.prepare(`
    SELECT IFNULL(SUM(b.${dtCol}), 0) AS downtime_hours
    FROM breakdowns b
      ...
      (doc) => {
        ...
        table(
          ...
          [
            ...
            { key: "severity", label: "Severity", width: 0.1 },
            { key: "hours", label: "Hours", width: 0.1, align: "right" },
            { key: "out", label: "OOS", width: 0.08, align: "center" },
            { key: "desc", label: "Description", width: 0.12 },
          ],
          rows.length
            ? rows.map((r) => ({
                id: String(r.id),
